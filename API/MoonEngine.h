...
namespace moon {
	...
	class CEngine {
		...
		std::shared_ptr<itpr::CScope> m_stdlibScope;
		std::map<std::string, std::shared_ptr<itpr::CScope>> m_units;
		...
		std::shared_ptr<itpr::CScope> m_GetUnit(const std::string& unitName);
