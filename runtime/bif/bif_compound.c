...
{
	err_push_virt("BIF", "Function _%s_ must produce homogenous result.", func);
}
...
{
	err_push_virt("BIF", "In _slice_ : %s", condition);
}

static void bif_cpd_error_mismatch(char *func)
{
	err_push_virt("BIF", "Arguments of _%s_ must be of matching types", func);
}
