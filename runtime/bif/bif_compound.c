...
{
	err_push("BIF", *eval_location_top(), "Function _%s_ must produce homogenous result.", func);
}
...
{
	err_push("BIF", *eval_location_top(), "In _slice_ : %s", condition);
}

static void bif_cpd_error_mismatch(char *func)
{
	err_push("BIF", *eval_location_top(), "Arguments of _%s_ must be of matching types", func);
}
