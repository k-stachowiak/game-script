...
#define BIF_ARYTHM_UNARY_DEF(NAME) \
    ...
    { \
        ...
        case VAL_REAL: \
            ...
            break; \
        default: \
			err_push_virt("BIF", "Arguments of arythmetic BIF must be of equal numeric type"); \
            break; \
        } \
    }
...
void bif_real(struct Runtime *rt, VAL_LOC_T x_loc)
{
	...
	if (rt_val_peek_type(rt, x_loc) != VAL_INT) {
		err_push_virt("BIF", "Arguments of expanding arythmetic BIF must be of integer type");
		return;
	}
