...
{
    enum ValueType lo_type = rt_val_peek_type(&rt->stack, lo_loc);
    enum ValueType hi_type = rt_val_peek_type(&rt->stack, hi_loc);
    VAL_INT_T lo;
    VAL_INT_T hi;
...
{
    enum ValueType p_type = rt_val_peek_type(&rt->stack, p_loc);
    VAL_REAL_T p;
    if (p_type != VAL_REAL) {
...
{
    enum ValueType l_type = rt_val_peek_type(&rt->stack, l_loc);
    VAL_REAL_T l;
    if (l_type != VAL_REAL) {
...
{
    enum ValueType u_type = rt_val_peek_type(&rt->stack, u_loc);
    enum ValueType s_type = rt_val_peek_type(&rt->stack, s_loc);
    VAL_REAL_T u;
    VAL_REAL_T s;
...
void bif_rand_distr(struct Runtime *rt, VAL_LOC_T d_loc)
{
    ...
    first_type = rt_val_peek_type(&rt->stack, loc);
